...
//        1.Сохранить в файл строку и загрузить из файла строку с выводом в консоль используя классы FileWriter и FileReader
        ...
        try {
            fileWriter = new FileWriter("HomeWork04.sql");
            fileWriter.append("Хвала гитхабу, stackoverflow и индии!");
            fileWriter.flush();
        } catch (IOException e) {
        ...
        String text = "";
        try {
            fileReader = new FileReader("HomeWork04.sql");
...
//        2.Загрузить из файла многострочный текст формата ФИО возраст и пол через пробелы. Разбить по строкам и вывести в консоль в формате "Иванов И.И. 32 М"
        String fileText = "";
        try {
            fileReader = new FileReader("Baza.sql");
            while (fileReader.ready()) {
